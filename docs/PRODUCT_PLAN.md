...
- 로컬 Git repository root 하위 프로젝트를 인식하고, 설정 화면에서 선택한 관리 대상만 현재 브랜치, rebase/pull 필요 여부, 오늘 내 커밋 기준으로 일일 작업 보고서에 사용
...
- Jira 일감 Slack 카드에서 관리 중인 로컬 repo를 선택해 브랜치를 바로 생성
