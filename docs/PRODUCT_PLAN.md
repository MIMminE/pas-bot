...
- 설정 마법사에서 GitHub 토큰으로 접근 가능한 원격 repository 목록 조회와 복수 선택
...
- `git_report`: snapshot 이후 내 커밋 기반 일일 보고서 또는 GitHub 원격 작업 리포트
...
- `repo remote-report`: 로컬 clone 없이 GitHub API만으로 오늘 내 커밋, 업데이트된 원격 브랜치, 내 미처리 PR 리포트 생성
- `repo remote-status`: 지정한 GitHub repository의 내 원격 브랜치를 기본 브랜치와 비교해 리베이스/동기화 필요 여부와 PR 연결 상태를 제안
- `repo remote-list`: GitHub 토큰으로 접근 가능한 repository 목록을 조회해 설정 마법사의 복수 선택 UI에 제공
