...
- 각 repository는 `base_branch`를 기준 브랜치로 사용합니다. 기준 브랜치가 아닌 브랜치는 작업중으로 표시하고, 기준 브랜치보다 뒤처진 작업 브랜치는 rebase 필요 상태로 표시합니다.
...
`config.toml`은 git에 커밋하지 않습니다. 릴리즈 앱을 실행하면 macOS 앱 데이터 폴더에 설정 파일을 만들고, macOS 메뉴바 앱은 시작할 때마다 설정 화면을 먼저 엽니다. 기존 로컬 설정이 있으면 입력값을 미리 채운 상태로 보여주며, 이후 업데이트에서도 같은 로컬 설정을 계속 사용합니다.
...
`state.json`에는 하루 1회 자동 실행 이력과 Jira 일감별 연결 repository가 저장됩니다. 예를 들어 `LMS-310`을 `lms-back`에 연결하면 다음 Jira 브리핑부터 해당 일감 카드에 연결 repo가 표시되고, 브랜치 시작 액션도 그 repo를 기준으로 동작합니다. 브랜치 시작은 처리되지 않은 로컬 변경이 없을 때만 실행되며, 설정된 `base_branch`를 최신화한 뒤 `feature/LMS-310-summary` 형태의 브랜치를 만듭니다.
...
macOS 앱은 실행 시 설정 화면을 열고 Slack Bot Token과 기능별 채널, Jira URL/email/API token/project, Git 작성자, GitHub repository clone 위치, OpenAI API key, 기능별 자동 실행 여부와 시간을 받습니다. 앱 메뉴에서 기존 `config.toml` 또는 `assignees.json`을 가져올 수 있습니다.
...
`v*` 태그를 push하면 GitHub Actions가 macOS 실행 파일이 포함된 zip을 만들고 GitHub 릴리즈에 업로드합니다.
...
릴리즈 zip에는 Python 런타임이 포함된 `bin/pas`가 들어가므로 사용자는 별도로 Python을 설치하지 않아도 됩니다.
