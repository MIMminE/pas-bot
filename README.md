...
- Jira 일감 Slack 카드에서 관리 중인 로컬 repo에 브랜치 생성 버튼 제공
...
- 설정 화면에서 인식된 Git 프로젝트 중 관리 대상만 복수 선택
...
- 설정 화면에서 로컬 Git repository root 폴더 지정
...
.\scripts\run-pas.ps1 dev create-branch --repo C:\Users\harun\Desktop\STL\lms-back --issue-key LMS-123 --summary "PDA 입하 캐시 개선"
...
scripts/run-pas.sh dev create-branch --repo ~/Desktop/STL/lms-back --issue-key LMS-123 --summary "PDA 입하 캐시 개선"
...
macOS 앱은 실행 시 설정 화면을 열고 Slack Bot Token과 기능별 채널, Jira URL/email/API token/project, Git 작성자, 로컬 Git repository root, OpenAI API key, 기능별 자동 실행 여부와 시간을 받습니다. macOS와 Windows 모두 앱 메뉴에서 기존 `config.toml` 또는 `assignees.json`을 가져올 수 있습니다.
