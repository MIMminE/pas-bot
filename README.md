...
- 기능별 Slack 목적지 라우팅: Webhook 직접 입력 또는 Slack Bot Token 기반 채널 선택
...
- GitHub 원격 브랜치/커밋/PR 기반 작업 리포트 생성
- GitHub 원격 브랜치의 ahead/behind, 리베이스 필요 여부, PR 연결 상태 점검
- 설정 마법사에서 GitHub 토큰으로 원격 repository 목록 조회 후 복수 선택
...
PAS는 로컬 앱 데이터 폴더의 `config.toml` 하나에 설정을 저장합니다. Slack Webhook URL 또는 Bot Token, Jira API 토큰, OpenAI API 키도 여기에 들어갑니다.
...
`[slack].mode = "webhook"`이면 `[slack].webhook_url` 또는 `[slack.webhooks]`의 기능별 웹훅을 사용합니다. `[slack].mode = "oauth"`이면 `[slack].bot_token`과 `[slack.channels]`의 채널 ID로 Slack Web API `chat.postMessage`를 사용합니다. macOS 설정 마법사에서는 Bot Token 입력 후 채널 목록을 불러와 기능별 채널을 선택할 수 있습니다.
...
.\scripts\run-pas.ps1 repo remote-report --send-slack
.\scripts\run-pas.ps1 repo remote-status --send-slack
.\scripts\run-pas.ps1 repo remote-list
...
scripts/run-pas.sh repo remote-report --send-slack
scripts/run-pas.sh repo remote-status --send-slack
scripts/run-pas.sh repo remote-list
...
macOS 앱은 실행 시 설정 마법사를 열고 Slack 연결 방식, 기능별 Slack Webhook 또는 OAuth 채널, Jira URL/email/API token/project, Git 작성자, GitHub token, 관리할 원격 GitHub repository, OpenAI API key, 기능별 자동 실행 여부와 시간을 받습니다. macOS와 Windows 모두 앱 메뉴에서 기존 `config.toml` 또는 `assignees.json`을 가져올 수 있습니다.
