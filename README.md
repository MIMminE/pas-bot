...
macOS 앱은 실행 시 설정 마법사를 열고 Slack 연결 방식, 기능별 Slack Webhook 또는 OAuth 채널, Jira URL/email/API token/project, Git 작성자, GitHub token, OpenAI API key, 기능별 자동 실행 여부와 시간을 받습니다. macOS와 Windows 모두 앱 메뉴에서 기존 `config.toml` 또는 `assignees.json`을 가져올 수 있습니다.
